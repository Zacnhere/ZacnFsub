...
from PremSub.config import MONGO_URL
from PremSub.config import NamaData
mongo_client = AsyncIOMotorClient(MONGO_URL)

mongodb = mongo_client.viihash_fsub
...
#haku
async def add_haku(user_id):
    await hakudb.insert_one({"_id": user_id})


async def del_haku(user_id):
    await hakudb.delete_one({"_id": user_id})


async def cek_haku():
    user_ids = []
    async for ids in hakudb.find({"_id": {"$gt": 0}}):
        user_id = ids["_id"]
        user_ids.append(user_id)
    return user_ids


async def haku_info(user_id):
    g = await hakudb.find_one({"_id": user_id})
    return g if g else False